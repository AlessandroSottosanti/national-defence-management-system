...
public class DocumentService extends AbstractService<Documents, DocumentsDto> {
    ...
    public List<DocumentsDto> index(DocumentFilterDto filter) {
        ...
        Specification<Documents> specification = (root, query, cb) -> {
            ...
            if (filter.getOperatorFirstName() != null) {

                predicates.add(
                        cb.like(
                                root.get("operator").get("firstName"),
                                filter.getOperatorFirstName()
                        )
                );
            }

            if (filter.getOperatorLastName() != null) {

                predicates.add(
                        cb.like(
                                root.get("operator").get("lastName"),
                                filter.getOperatorLastName()
                        )
                );
            }

            if (filter.getOperatorServiceNumber() != null) {

                predicates.add(
                        cb.like(
                                root.get("operator").get("serviceNumber"),
                                filter.getOperatorServiceNumber()
                        )
                );
            }
